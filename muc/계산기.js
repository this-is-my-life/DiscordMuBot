...
module.exports.run = async (mu, input, pars) => {
	...
	input.channel.send("입력 받을 값의 진수를 입력하세요 (2진수, 10진수, 16진수 지원)\n\"취소\"를 입력하거나 30초를 쓸때없이 기다리시면 뮤봇이 빡쳐서 취소할껍니다").then(this => this.delete(30000));
	input.channel.awaitMessages(filter, {
		max: 1,
		time: 30000
	}).then(collected => {
		let col1input = collected.first().content;
		if (col1input === "취소") { input.channel.send("취소되었습니다").then(this => this.delete(2000)); } else {
			...
			input.channel.awaitMessages(filter, {
				...
				if (col2input === "취소") { input.channel.send("취소되었습니다").then(this => this.delete(2000)); } else {
